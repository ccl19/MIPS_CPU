...
iverilog -g 2012\
    ...
    -P mips_cpu_pipeline_tb.MEM_1=\"./test/1-hex/mem_1/AUTO_${INSTRUCTION}/${TESTCASE}.txt\" \
    -P mips_cpu_pipeline_tb.MEM_2=\"./test/1-hex/mem_2/AUTO_${INSTRUCTION}/${TESTCASE}.txt\" \
...
if [[ "${RESULT}" -ne 0 ]] ; then
    echo "${TESTCASE} ${INSTRUCTION} Fail"   
    echo "${TESTCASE} ${INSTRUCTION} Fail" >> test/RESULT_PIPELINE.out
    exit 1
else
    echo "${TESTCASE} ${INSTRUCTION} Pass" 
    echo "${TESTCASE} ${INSTRUCTION} Pass" >> test/RESULT_PIPELINE.out
    exit 0
fi